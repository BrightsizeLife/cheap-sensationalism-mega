...
/** A subway bullet: the line's letter in a circle. Always sits next to the
 *  line's name, so it is hidden from screen readers unless `label` is set. */
export function Bullet({ line, label = false, size }: { line: Line; label?: boolean; size?: 'lg' }) {
  return (
    <span
      className={`cs-bullet${size === 'lg' ? ' cs-bullet-lg' : ''}`}
      data-hue={line.hue}
      {...(label ? { role: 'img', 'aria-label': `${line.name} line` } : { 'aria-hidden': true })}
    >
      {line.letter}
    </span>
  );
...
export function Bullets({ atlas, thing }: { atlas: Atlas; thing: Thing }) {
  ...
  return (
    <span className="cs-bullets">
      ...
      <span className="pact-sr-only">{lines.map((l) => l.name).join(' and ')} line</span>
    </span>
  );
}
...
export function Go({ link, className = 'pact-cta', children }: { link: LinkRef; className?: string; children?: React.ReactNode }) {
  const body = children ?? link.label;
  if (isExternal(link.url) || link.url.endsWith('.html')) {
    return (
      <a className={className} href={link.url} rel="noopener">
        {body}
      </a>
    );
  }
